...
-(void)openFileAtIndex:(NSInteger)index{
    ...
    NSString *path=[[HGBFileManageTool getHomeFilePath]stringByAppendingPathComponent:fileModel.filePath];
...
-(void)alertToolSelectViewWithFlag:(BOOL)flag{
    ...
    if(flag){
        ...
        if(fileModel.fileType==HGBFileTypeDirectory||fileModel.fileType==HGBFileTypeBundle){
             self.toolsArr=[NSMutableArray arrayWithArray:@[@{@"id":@"0",@"title":@"打开"}]];
        }else{
             self.toolsArr=[NSMutableArray arrayWithArray:@[@{@"id":@"0",@"title":@"打开"},@{@"id":@"3",@"title":@"使用浏览器打开"},@{@"id":@"4",@"title":@"其他方式打开"}]];
        }
    ...
    for(NSDictionary *dic in self.toolsArr){
        UIAlertAction *action=[UIAlertAction actionWithTitle:dic[@"title"] style:(UIAlertActionStyleDefault) handler:^(UIAlertAction * _Nonnull action) {
            ...
            if (idString.integerValue==0) {
                ...
                [HGBFileWebLook lookFileAtPath:[[HGBFileManageTool getHomeFilePath] stringByAppendingPathComponent:fileModel.filePath] inParent:self];
            }else if(idString.integerValue==4){
                 [HGBFileOutAppOpenFileTool openFileWithExetenAppWithPath:[[HGBFileManageTool getHomeFilePath] stringByAppendingPathComponent:fileModel.filePath] inParent:self andWithCompleteBlock:^(NSInteger status) {
                ...
                UIAlertAction *confirmAction = [UIAlertAction actionWithTitle:@"重命名" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
                    ...
                    NSString *curentPath=[[HGBFileManageTool getHomeFilePath]stringByAppendingPathComponent:fileModel.filePath];
                ...
                self.sourceDataPath=[[HGBFileManageTool getHomeFilePath]stringByAppendingPathComponent:fileModel.filePath];
