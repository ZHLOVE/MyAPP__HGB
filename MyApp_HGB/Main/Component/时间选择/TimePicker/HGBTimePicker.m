...
-(void)viewSetUp{
    ...
    self.promptLab.textColor=[UIColor blueColor];
...
#pragma mark 视图消失
-(void)popViewDisappearWithSucessBlock:(void (^)())successBlock
{
    ...
    if(self.type==HGBTimePickerTypeNO)
    {
        if((!self.startTime)&&(!self.stopTime)){
            if(self.numberOfHours<0){
                ...
                self.startTime=[self.f dateFromString:[NSString stringWithFormat:@"%d%@%@",currentHour.integerValue+self.numberOfHours,curentMin,curentSecond]];
            }else if(self.numberOfHours>0){
                self.startTime=[NSDate date];
                self.stopTime=[self.f dateFromString:[NSString stringWithFormat:@"%d%@%@",currentHour.integerValue+self.numberOfHours,curentMin,curentSecond]];
